...
public:
  ...
  bool try_lock() {
    return !(
      m_flag.load(std::memory_order::relaxed) ||
...
//! A class that locks and unlocks a given spinlock in an RAII-fashion.
...
#pragma ide diagnostic ignored "google-explicit-constructor"
  explicit(false) scoped_spinlock(spinlock& spinlock):
#pragma clang diagnostic pop
    m_spinlock_ref(spinlock) {
    m_spinlock_ref.lock();
  }

  ~scoped_spinlock() { m_spinlock_ref.unlock(); }
